...
// ================================================================
 async function showSuccessWithCheckmark(title, message, details) {
    console.log('🔵 showSuccessWithCheckmark called');
    ...
    if (popup2) {
        ...
        console.warn('⚠️ depositPopup2 not found');
    }
    ...
    setTimeout(function() {
        var popup3 = document.getElementById('depositPopup3');
        if (popup3) {
            popup3.style.display = 'flex';
            popup3.classList.remove('hidden');
            
            var amountEl = document.getElementById('successAmount');
            var methodEl = document.getElementById('successMethod');
            if (amountEl) amountEl.textContent = (amount || 0).toLocaleString() + ' KS';
            if (methodEl) methodEl.textContent = method || 'KBZ Pay';
            
            console.log('✅ Popup 3 opened');
        } else {
            console.error('❌ depositPopup3 not found!');
            if (typeof window.showNotification === 'function') {
                window.showNotification('✅ ငွေသွင်းပြီးပါပြီ။', 'success');
            }
        }
    }, 300);
}
