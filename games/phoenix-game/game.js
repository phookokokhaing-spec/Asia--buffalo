...
function notifyBetToLobby(amount) {
    sendToLobby({ type: 'BET_DEDUCT', amount, game: 'phoenixGame' });
}

function notifyWinToLobby(amount) {
    sendToLobby({ type: 'WIN_AMOUNT', amount, game: 'phoenixGame' });
}

function requestBalanceFromLobby() {
    sendToLobby({ type: 'REQUEST_BALANCE', game: 'phoenixGame' });
}

function syncBalanceToLobby(balance) {
    sendToLobby({ type: 'SYNC_BALANCE', balance, game: 'phoenixGame' });
}
...
// ---- Image Paths ----
const IMAGE_PATHS = {
    tiger: 'images/sym_tiger.png',
    jade: 'images/sym_jade.png',
    dragon: 'images/sym_dragon.png',
    turtle: 'images/sym_turtle.png',
    lotus: 'images/sym_lotus.png',
    phoenix: 'images/sym_phoenix.png',
    ingot: 'images/sym_ingot.png',

    deern: 'images/sym_deern.png',
    nlion: 'images/sym_nlion.png',
    buffalon: 'images/sym_buffalon.png',
    eagle: 'images/sym_eagle.png',

    n9: 'images/sym_n9.png',
    na: 'images/sym_na.png',
    nten: 'images/sym_nten.png',

    wilds: 'images/sym_wilds.png',
    scatter: 'images/sym-scatter.png'
};
// ============================================
// PAYTABLE (HARD MODE)
// ============================================
...
function buildPool(reelIndex, rules = null) {
    ...
    for (const [sym, count] of Object.entries(WEIGHTS)) {
        if (sym === 'wilds' && (reelIndex === 0 || reelIndex === 4)) continue;
        if (sym === 'wilds' && isCh1 && reelIndex === 1) continue;
...
function applyScaleCheck(reels, rules) {
    ...
    for (let i = 0; i < r2.length; i++) {
        if (r2[i] === 'wilds' || r2[i] === 'scatter') continue;

        const r1Regular = r1.filter(s => s !== 'wilds' && s !== 'scatter');

        if (r1Regular.includes(r2[i])) {
            const pool = buildPool(crossCheck.reel2, rules).filter(s => {
                if (s === 'wilds' || s === 'scatter') return true;
...
window.addEventListener('message', function(event) {
    ...
    if (data.type === 'SYNC_BALANCE' && data.game === 'phoenixGame') {
    ...
    if (data.type === 'REQUEST_BALANCE' && data.game === 'phoenixGame') {
